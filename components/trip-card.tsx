...
export function TripCard({ trip, isSelected, onSelect }: TripCardProps) {
  const VehicleIcon = vehicleIcons[trip.vehicleType as keyof typeof vehicleIcons] || Car
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <Avatar className="h-10 w-10">
                <AvatarImage src={trip.publisher.avatar || "/placeholder.svg"} alt={trip.publisher.name} />
                <AvatarFallback>
                  {trip.publisher.name
                    .split(" ")
                    .map((n) => n[0])
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <span className="font-medium">{trip.publisher.name}</span>
