...
export const TripsService = {
  ...
  complete: (id: string) => api(`/api/trips/${id}/complete`, { method: "PATCH" }),
};
...
export const ChatService = {
  ...
  between: (userAId: string, userBId: string, create = true) =>
    api(`/api/chat/between?userAId=${userAId}&userBId=${userBId}&create=${create ? 1 : 0}`),
  messages: (chatId: string) => api(`/api/chat/${chatId}/messages`),
  postMessage: (chatId: string, payload: any) =>
    api(`/api/chat/${chatId}/messages`, { method: "POST", body: JSON.stringify(payload) }),
};
